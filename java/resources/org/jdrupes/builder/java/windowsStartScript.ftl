...
endlocal & "%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %${optsEnvironmentVar}% <#if appNameSystemProperty??>"-D${appNameSystemProperty}=%APP_BASE_NAME%"</#if><#if classpath??> -classpath "%CLASSPATH%"</#if> <#if modulePath??>--module-path "%MODULE_PATH%" </#if>${mainClassName!entryPointArgs} %* & call :exitWithErrorLevel
